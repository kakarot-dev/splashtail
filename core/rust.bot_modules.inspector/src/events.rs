        ...
        AntiraidEvent::Discord(ref event) => {
            match event {
                FullEvent::Message { new_message } => {
                    let data = &ectx.data;
                    let config =
                        super::cache::get_specific_configs(&data.pool, ectx.guild_id).await?;
                ...
                FullEvent::GuildMemberAddition { new_member } => {
                        ...
                        .contains(AutoResponseMemberJoinOptions::DISABLED)
                    {
                        ...
                        let cache_http =
                            botox::cache::CacheHttpImpl::from_ctx(&ectx.serenity_context);
                        let Some(bot) =
                                ...
                                match flag {
                                    ...
                                    AutoResponseMemberJoinOptions::KICK_NEW_MEMBERS => {
                                        ...
                                        new_member
                                            .kick(
                                                &ctx.http,
                                                Some("Auto response: Kick new members"),
                                            )
                                            .await?;
                                    ...
                                    AutoResponseMemberJoinOptions::BAN_NEW_MEMBERS => {
                                        ...
                                        new_member
                                            .ban(
                                                &ctx.http,
                                                0,
                                                Some("Auto response: Ban new members"),
                                            )
                                            .await?;
                            ...
                            .contains(FakeBotDetectionOptions::BLOCK_ALL_UNKNOWN_BOTS)
                        {
                            ...
                            let is_an_official_bot =
                                super::cache::FAKE_BOTS_CACHE.iter().any(|fb| {
                                    fb.value().official_bot_ids.contains(&new_member.user.id)
                                });
                                ...
                                .contains(FakeBotDetectionOptions::NORMALIZE_NAMES)
                            {
                                ...
                                for prefixes in
                                    ["premium", "vip", "prime", "pro", "official", "bot"].iter()
                                {
                                    if normalized_name.starts_with(prefixes) {
                                        normalized_name =
                                            normalized_name[prefixes.len()..].to_string();
                                    }
                                }
                                    ...
                                    ["premium", "vip", "prime", "pro", "official", "bot"].iter()
                                {
                                    if normalized_name.starts_with(suffixes) {
                                        normalized_name =
                                            normalized_name[suffixes.len()..].to_string();
                                    }
                                }
                            ...
                            for fb in super::cache::FAKE_BOTS_CACHE.iter() {
                                ...
                                {
                                    let (diff, _) = splashcore_text::diff::diff(
                                        &normalized_name,
                                        &val.name,
                                        "",
                                    );
                ...
                FullEvent::GuildUpdate {
                    ...
                    let config =
                        super::cache::get_global_config(&ectx.data.pool, ectx.guild_id).await?;
                        ...
                        .contains(GuildProtectionOptions::DISABLED)
                    {
                        ...
                        if !bp.manage_guild() {
                            return Err(format!(
                                "Cannot manage guild on this guild: {}",
                                ectx.guild_id
                            )
                            .into());
                        }
